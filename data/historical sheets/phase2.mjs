...
function readCard(rows){
  ...
  for(const r of rows){
    ...
    const isTeam=/[A-Z]/.test(team) && team.length>=2 && team.length<=12 && !/^\d/.test(team[0]);
...
const matchFacts=[];
function emit(round,format,a,b,teamA,statusA){
  matchFacts.push({year,round,format,player:a,opponent:b,team:teamA,
    ...
    won:statusA[17]>0, halved:statusA[17]===0});
...
function processCard(round,format,rows){
  ...
  if(format==="singles"){
    for(let i=0;i+1<players.length;i+=2){
      const da=base.find(d=>d.player===players[i].player);
      const db=base.find(d=>d.player===players[i+1].player);
      if(da&&db){
  ...
  for(const d of A) emit(round,format,d.player,B.map(x=>x.player).join("+"),teams[0],sA);
  for(const d of B) emit(round,format,d.player,A.map(x=>x.player).join("+"),teams[1],sA.map(x=>-x));
