...
// ══════════════════════════════════════════════════════════════════
//
// All three golf apps carry this hook, so its behaviour is worth pinning in each
// of them rather than trusting that a shared file stayed shared.
//
...
describe("dirty tracking", () => {
  it("starts clean", () => {
    const { latest } = mount({ name: "Cup" });
    expect(latest.isDirty).toBe(false);
    expect(latest.value).toEqual({ name: "Cup" });
  });

  it("goes dirty on an edit", () => {
    const { latest } = mount({ name: "Cup" });
    act(() => latest.setValue({ name: "Cup 2026" }));
...
describe("save", () => {
  it("hands onSave the current value", async () => {
    const onSave = vi.fn();
    const { latest } = mount({ name: "Cup" }, onSave);
    act(() => latest.setValue({ name: "Cup 2026" }));
    await act(() => latest.save());
    expect(onSave).toHaveBeenCalledWith({ name: "Cup 2026" });
  });
...
describe("syncing from upstream", () => {
  it("takes an incoming document while the form is clean", () => {
    const { latest, rerender } = mount({ name: "Cup" });
  ...
  it("does not revert to the stale prop after a save", async () => {
    ...
    act(() => latest.setValue({ name: "Cup 2026" }));
    await act(() => latest.save());
    expect(latest.value).toEqual({ name: "Cup 2026" });
    expect(latest.isDirty).toBe(false);
  });
