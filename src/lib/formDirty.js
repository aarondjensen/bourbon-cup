...
// The player sheet. `dir` is the crown and `cap` the armband — both write to
// bc_accounts rather than the roster row, and both are committed by the same
// Save, so they belong here.
//
// So do the two sign-in edits, `unlink` and `linkUid`, and they were missing.
// Nothing broke — the Save button is never disabled, only lit — so cutting a
// player's sign-in left the button grey while it sat there as a real pending
// change, which is the one signal this sheet gives that there is something
// to commit.
const PLAYER_FIELDS = ["team", "first", "last", "nick", "hi", "ov", "dir", "cap", "wd",
  "unlink", "linkUid",
...
export function playerFormSig(form) {
  ...
  return JSON.stringify(PLAYER_FIELDS.map(k => {
    const v = form[k];
    if (k === "dir" || k === "cap" || k === "wd" || k === "unlink") return !!v;
