...
| 1.4 Reviewer account | **Not needed** | No credentials are handed to either store. `bc_demo` holds its documents and all sixteen roster rows, unclaimed; a reviewer signs in with their own Apple ID or Google account, presents the REVIEWER code (not the tournament password — see below), claims a name, and gets the Admin tabs because the edition is a demo. |
