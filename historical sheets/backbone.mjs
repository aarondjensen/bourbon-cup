...
const FMT = f => { const n=norm(f); return n.includes("dot")||n.includes("dozen")?"dots"
...
function extractMaster(rows){
  ...
  if(r){ for(let rd=1;rd<=4;rd++){ const c=r[4+(rd-1)*2], f=r[5+(rd-1)*2];
    if(f && norm(f).match(/dozen|singles|scramble|bestball|hilo|dot/))
...
writeFileSync("/mnt/user-data/outputs/bourbon-cup-backbone.json",JSON.stringify(all,null,2));
console.log(`\n→ bourbon-cup-backbone.json (${all.playerHole.length} hole facts, ${all.playerRound.length} round facts)`+(issues?` · ${issues} issues`:" · clean"));