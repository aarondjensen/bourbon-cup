...
// ══════════════════════════════════════════════════════════════════
//
// `demo_code` on bc_secrets/access is a SECOND password that mints a
// membership scoped to the demo tournament. It exists because the tournament
// password goes to a dozen Play testers and to two review queues, and the
// thing that made that survivable — every real edition being locked — expires
// the week the cup is played. On that morning, without this, every membership
// ever minted for a store review becomes a writer on the live tournament.
//
// The property that matters is the last one in this block: a demo member is
// refused on a real edition even when that edition is UNLOCKED.
await env.clearFirestore();
await seed("bc_secrets/access", { code: "Bourbon2026", demo_code: "TesterPass" });
await seed("bc_editions/bc_demo", { id: "bc_demo", is_demo: true });
await seed("bc_editions/bc_2026", { id: "bc_2026" });

await check("the tester password mints a demo-scoped membership", () =>
  assertSucceeds(setDoc(doc(peteDb(), "bc_accounts/pete"),
    { uid: "pete", code: "testerpass", scope: "demo" })));

await check("the tournament password still mints an ordinary one", () =>
  assertSucceeds(setDoc(doc(aliceDb(), "bc_accounts/alice"),
    { uid: "alice", code: "bourbon2026" })));

await check("the two doors cannot be crossed", async () => {
  // The tester password asking for a full membership is the whole point.
  await assertFails(setDoc(doc(malloryDb(), "bc_accounts/mallory"),
    { uid: "mallory", code: "TesterPass" }));
  await assertFails(setDoc(doc(malloryDb(), "bc_accounts/mallory"),
    { uid: "mallory", code: "TesterPass", scope: "full" }));
  // And the tournament password cannot mint a demo one, which would let
  // somebody scope themselves out of a lock they are subject to.
  await assertFails(setDoc(doc(malloryDb(), "bc_accounts/mallory"),
    { uid: "mallory", code: "Bourbon2026", scope: "demo" }));
});

await check("a wrong password still opens neither", () =>
  assertFails(setDoc(doc(malloryDb(), "bc_accounts/mallory"),
    { uid: "mallory", code: "guess", scope: "demo" })));

await check("a demo member can score IN the demo tournament", () =>
  assertSucceeds(setDoc(doc(peteDb(), "bc_hole_scores/d1"),
    { tournament_id: "bc_demo", v: 4 })));

await check("a demo member CANNOT score in the real cup — unlocked", async () => {
  // Unlocked deliberately: the lock is what protected this until now, and the
  // lock comes off the week the tournament is played.
  await assertFails(setDoc(doc(peteDb(), "bc_hole_scores/r1"),
    { tournament_id: "bc_2026", v: 4 }));
});

await check("a demo member cannot carry a row out of the demo into the cup", async () => {
  await seed("bc_hole_scores/d2", { tournament_id: "bc_demo", v: 4 });
  await assertFails(setDoc(doc(peteDb(), "bc_hole_scores/d2"),
    { tournament_id: "bc_2026", v: 4 }));
});

await check("a demo member cannot claim a name on the real roster", async () => {
  await seed("bc_players/real1", { tournament_id: "bc_2026", name: "Aaron J", auth_uid: null });
  await assertFails(setDoc(doc(peteDb(), "bc_players/real1"),
    { tournament_id: "bc_2026", name: "Aaron J", auth_uid: "pete" }, { merge: true }));
});

await check("a demo member CAN still register their own phone for push", () =>
  // Not edition-scoped — a push token belongs to a device, and a tester who
  // cannot receive a notification is a tester who generates nothing.
  assertSucceeds(setDoc(doc(peteDb(), "bc_notification_tokens/pt1"),
    { player_id: "demo_dave", token: "t" })));

await check("an ordinary member is untouched by any of it", async () => {
  await assertSucceeds(setDoc(doc(aliceDb(), "bc_hole_scores/r2"),
    { tournament_id: "bc_2026", v: 5 }));
  await assertSucceeds(setDoc(doc(aliceDb(), "bc_hole_scores/d3"),
    { tournament_id: "bc_demo", v: 5 }));
});

await check("a membership with no scope field is a full one", async () => {
  // Every membership that existed before this did. If this fails, deploying
  // the rules evicts the entire field.
  await env.withSecurityRulesDisabled(ctx =>
    setDoc(doc(ctx.firestore(), "bc_accounts/legacy"), { uid: "legacy" }));
  const legacyDb = env.authenticatedContext("legacy").firestore();
  await assertSucceeds(setDoc(doc(legacyDb, "bc_hole_scores/r3"),
    { tournament_id: "bc_2026", v: 6 }));
});

// ── With no tester password configured ──────────────────────────────
await env.clearFirestore();
await seed("bc_secrets/access", { code: "Bourbon2026" });

await check("no tester password means no second door", () =>
  assertFails(setDoc(doc(malloryDb(), "bc_accounts/mallory"),
    { uid: "mallory", code: "", scope: "demo" })));

await check("and a tester password equal to the real one opens neither as demo", async () => {
  await seed("bc_secrets/access", { code: "Bourbon2026", demo_code: "bourbon2026" });
  // One string cannot mean two things — the scope it minted would be whichever
